...
saveIndex = function(run,file){
  ...
  if(run$conf_l$applyALK==1){
    ...
    for(i in 1:length(conf_l$years)){
      id = i + (0:(run$data$ageRange[2]-1))*length(conf_l$years)
    ...
    covYears = list()
    for(i in 1:length(conf_l$years)){
      id = i + (0:(run$data$numberOfLengthGroups-1))*length(conf_l$years)
      covYears[[i]] = cov[id,id]
      covYears[[i]] = covYears[[i]]
    }
  }

  write.table(ageIndex, file = file) #Save index
  write.table(ageLogIndexSd, file = paste0("sd", file)) #Save sd of index

  fileNameCov = paste0("cov_",strsplit(file,'[.]')[[1]][1],".Rda")
