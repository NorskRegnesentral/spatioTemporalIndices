...
template <class Type>
  array<Type> ALK(dataSet<Type> dat, paraSet<Type> par){
    ...
    Type sigma_alk =exp(par.logSigma_alk(0));
    Type kappa_alk =exp(par.logKappa_alk(0));
    Type scaleST_alk = Type(1)/((4*3.14159265)*kappa_alk*kappa_alk);
    ...
    for(int a =0; a<(nAges-1); ++a){
      for(int l = 0; l<dat.numberOfLengthGroups; ++l){
        for(int y=0; y<nYears; ++y){
          vector<Type> delta_alk = (dat.Apred_alk*par.xST_alk.col(a).col(y).matrix())/sqrt(scaleST_alk);
          linPredArray_alk_int.col(a).col(l).col(y) = par.beta0_alk(y,a) +
              par.betaLength_alk(a)*(dat.lengthGroups(l) + 0.5*dat.dL(0)) +delta_alk*sigma_alk;
        }
      }
    }
