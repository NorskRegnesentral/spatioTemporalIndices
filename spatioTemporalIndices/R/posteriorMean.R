...
##' @details Return posterior mean of the index (using the bias-correction flag in TMB::sdreport for the catch-at-length random effects only)
##' @export
posteriorMean = function(run){
  ...
  if(run$conf_l$applyALK ==0){
    ...
    sdrep <- TMB::sdreport(run$obj,bias.correct = TRUE,
